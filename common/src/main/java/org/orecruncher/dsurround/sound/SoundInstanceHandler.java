...
public final class SoundInstanceHandler {
    ...
    static {
        ...
        THUNDER_SOUND = SoundFactoryBuilder.create(new Identifier(DynamicSurroundings.MOD_ID, "thunder"))
    ...
    private static boolean isSoundBlocked(final Identifier id) {
        return DynamicSurroundings.SOUND_CONFIG.isBlocked(id);
    }

    private static boolean isSoundCulled(final Identifier id) {
        return DynamicSurroundings.SOUND_CONFIG.isCulled(id);
    }

    private static boolean isSoundCulledLogical(final Identifier sound) {
        int cullInterval = DynamicSurroundings.CONFIG.soundSystem.cullInterval;
    ...
    public static boolean shouldBlockSoundPlay(final SoundInstance theSound) {
        ...
        if (DynamicSurroundings.CONFIG.thunderStorms.replaceThunderSounds && thunderSounds.contains(id)) {

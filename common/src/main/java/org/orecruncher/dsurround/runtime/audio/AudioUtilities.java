...
import org.orecruncher.dsurround.mixin.audio.MixinSoundEngineAccessor;
import org.orecruncher.dsurround.mixin.core.MixinAbstractSoundInstance;
import org.orecruncher.dsurround.mixin.core.MixinSoundManagerAccessor;
import org.orecruncher.dsurround.mixin.core.MixinSoundSystemAccessors;

public final class AudioUtilities {
    private static final IModLog LOGGER = DynamicSurroundings.LOGGER.createChild(AudioUtilities.class);
        ...
        try {
            ...
            if (doEnhancedSounds()) {
                ...
                if (!hasFX) {
                    ...
                    var freq = DynamicSurroundings.CONFIG.enhancedSounds.outputFrequency;
                    if (freq != 0) {
                        DynamicSurroundings.LOGGER.info("Attempting to set output frequency of %d", freq);
                    ...
                    if (deviceCaps.ALC_SOFT_HRTF) {
                        ...
                        if (status == SOFTHRTF.ALC_HRTF_DISABLED_SOFT && DynamicSurroundings.CONFIG.enhancedSounds.enableHRTF) {
    ...
    private static boolean doEnhancedSounds() {
        if (!DynamicSurroundings.CONFIG.enhancedSounds.enableEnhancedSounds) {

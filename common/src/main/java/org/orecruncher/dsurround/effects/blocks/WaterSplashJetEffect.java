...
public class WaterSplashJetEffect extends ParticleJetEffect {
    ...
    static {
        var factory = SoundFactoryBuilder.create(new Identifier(DynamicSurroundings.MOD_ID, "waterfall.0"))
                .pitchRange(0.8F, 1.2F)
                .build();
        Arrays.fill(waterfallAcoustics, factory);

        factory = SoundFactoryBuilder.create(new Identifier(DynamicSurroundings.MOD_ID, "waterfall.1"))
                .pitchRange(0.8F, 1.2F)
                .build();
        waterfallAcoustics[2] = waterfallAcoustics[3] = factory;

        factory = SoundFactoryBuilder.create(new Identifier(DynamicSurroundings.MOD_ID, "waterfall.2"))
                .pitchRange(0.8F, 1.2F)
                .build();
        waterfallAcoustics[4] = factory;

        factory = SoundFactoryBuilder.create(new Identifier(DynamicSurroundings.MOD_ID, "waterfall.3"))
                .pitchRange(0.8F, 1.2F)
                .build();
        waterfallAcoustics[5] = waterfallAcoustics[6] = factory;

        factory = SoundFactoryBuilder.create(new Identifier(DynamicSurroundings.MOD_ID, "waterfall.4"))
                .pitchRange(0.8F, 1.2F)
                .build();
        waterfallAcoustics[7] = waterfallAcoustics[8] = factory;

        factory = SoundFactoryBuilder.create(new Identifier(DynamicSurroundings.MOD_ID, "waterfall.5"))
                .pitchRange(0.8F, 1.2F)
                .build();
    ...
    protected void soundUpdate() {
        if (!DynamicSurroundings.CONFIG.blockEffects.enableWaterfallSounds) {
            if (this.sound != null) {
    ...
    protected void spawnJetParticle() {
        if (!DynamicSurroundings.CONFIG.blockEffects.enableWaterfallParticles)
            return;
